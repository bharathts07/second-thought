...
/**
 * A section heading for banded layouts.
 *
 * Replaces the numbered ProseSection. With bands providing the visual rhythm,
 * the section heading is just the heading itself, no numeral or decorative rule.
 * The Band component handles spacing and background color.
 */
export function ProseSectionHeading({
  children,
  id,
}: {
  children: ReactNode;
  ...
  return (
    <h2
      id={id}
      className={`${MEASURE} mb-5 text-xl font-semibold tracking-tight text-ink`}
    >
      {children}
    </h2>
  );
